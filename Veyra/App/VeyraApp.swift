...
struct VeyraApp: App {
    ...
    init() {
        ...
        do {
            try VeyraModelContainer.seedDefaultCategoriesIfNeeded(in: container.modelContainer.mainContext)
        } catch {
            AppLogger.error("Failed to seed default categories: \(error.localizedDescription)")
        }

        AppLogger.debug("VeyraApp initialized. Theme loaded: \(self.settings.theme)")
    }
...
#Preview {
    ContentView(settings: .constant(.default))
        .modelContainer(VeyraModelContainer.preview)
}
